...
def get_model(model_name="MobileNetV2"):
    ...
    prediction_g = Dense(units=2, kernel_initializer="he_normal", use_bias=False, activation="softmax",
                       name="pred_gen")(base_model.output)
    prediction_a = Dense(units=101, kernel_initializer="he_normal", use_bias=False, activation="softmax",
                       name="pred_age")(base_model.output)

    model = Model(inputs=base_model.input, outputs=[prediction_g, prediction_a])
