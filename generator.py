...
class FaceGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i, sample_id in enumerate(sample_indices):
            ...
            g[i]=gender
            #print(y[i])
        return x, [to_categorical(g, 2),to_categorical(y, 101)]
    ...
    def _load_appa(self, appa_dir):
        ...
        gt_train_path = appa_root.joinpath("train_0917_asia_mega_allage.csv")
        df = pd.read_csv(str(gt_train_path))

        for i, row in df.iterrows():
            #if i%10!=8 and i%10!=9:
                age = min(100, int(row.age))
                gender = int(row.gender)
                # age = int(row.real_age)
                image_path = train_image_dir.joinpath(row.file_name)

                if image_path.is_file():
                    self.image_path_and_age.append([str(image_path), age,gender])
...
class ValGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(batch_size):
            image_path, age ,gender= self.image_path_and_age[idx * batch_size + i]
            image = cv2.imread(str(image_path))
    ...
    def _load_appa(self, appa_dir):
        appa_root = Path(appa_dir)
        val_image_dir = appa_root.joinpath("train")
        gt_val_path = appa_root.joinpath("test_0917_asia_mega_allage.csv")
        ...
        for i, row in df.iterrows():
                ...
                # age = int(row.age)
                gender = int(row.gender)
                image_path = val_image_dir.joinpath(row.file_name)
